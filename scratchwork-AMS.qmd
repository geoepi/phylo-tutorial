...
# save to text file - fasta format  
writeLines(sat2_sequences, here("data_AMS/sat2_sequences.fasta"))
```
...
# save the aligned sequences to a text file  
writeXStringSet(alignment2, filepath = here("data_AMS/aligned_sat2.fasta"))
```
...
alignment2 <- read.dna(here("data_AMS/aligned_sat2.fasta"),
                      format="fasta", as.matrix=TRUE)
...
write.table(sat2_dates, file = here("beast_AMS/sat2_dates.txt"), 
            sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
```
