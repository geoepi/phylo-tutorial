...
writeLines(sat1_sequences, here("data_AMS/sat1_sequences.fasta"))
```
...
writeXStringSet(alignment, filepath = here("data_AMS/aligned_sat1.fasta"))
```
...
```{r message=FALSE}
alignment <- read.dna(here("data_AMS/aligned_SAT1.fasta"),
...
write.table(sat1_dates, file = here("beast_AMS/sat1_dates.txt"), 
...
SAT1 files
- Ceres: shell file runs and returns files, but Beagle fails to load
- Atlas: ditto
...
```
not sure how to load Beast2 properly, can't run "beast" line with above code ("/software/el9/apps/beast2/2.7.3/bin/beast: line 67: /software/el9/apps/java/17/bin/java: Operation not permitted") and it appears to load the wrong version but specifying /2.7.7 also doesn't work
...
$ treeannotator -burnin 10000 -heights median sat1-treelog-AMS.trees sat1-mcc.tree
```
last line doesn't work ("/home/annakate.schatz/beast/bin/treeannotator: line 33: /apps/spack-managed/gcc-11.3.1/beast2-2.7.7-jiva66x7qqmcbrniff4jtdqspeg7ao2q/jre/bin/java: No such file or directory")
